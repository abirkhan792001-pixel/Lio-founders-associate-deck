# 19 · September 2026 Action Sheet

← [Trigger-Group Outreach](18-trigger-group-outreach-angles.md) | [Index](00-INDEX.md) | [Event Strategy →](20-event-strategy-and-icp-baseline.md)

> Published page: **[September Action Sheet](https://claude.ai/code/artifact/e0e2139d-2b9b-48bf-a701-97c5f9d03353)**. Generated 2026-08-31 from the refreshed [Target Event Pipeline](Lio_Target_Event_Pipeline.csv) and [P1 Budget & Staffing plan](Lio_P1_Budget_Staffing.csv), then **reconciled against the organizer-verified dates in [16-event-status-update.md](16-event-status-update.md)**. 29 actions, ordered by due date rather than priority tier.

## What is at stake this month

| | |
|---|---|
| **€165K** | new spend still unbooked across the four P1 September events |
| **€70K** | already sunk, the ProcureCon East lead-sponsor fee |
| **13 events + 2 hard deadlines** | inside the next 30 days |
| **65** | on-site staff-days planned for the quarter |

## Three calls that cannot slip

1. **Two early-birds close Sep 30, not one.** The BME-eLösungstage 2027 partner tier (€300 off. Stäbe) *and* the 61. BME-Symposium early-bird (€100 off. Wagner). The July plan had the Symposium as a soft "book in September" item; the organizer check found a hard cliff. Both are renewals where Lio is already established, so neither is a debate, they just have to be signed.
2. **Two bundle windows close before the events they cover.** The WBR two-event bundle (Cologne + Boston) expires with **Cologne on Sep 22**; the Executive Platforms bundle (NAPES + NASCES) must be agreed before **NASCES runs Sep 21-23**. Both were written down as Q4 decisions and September overtook them. **The second now has a prerequisite:** NAPES' reported move to Denver is unverified, and the city has to be settled before ~€35K is committed. *(Keil / Petras, due Sep 8-10.)*
3. **The October split is settled, one condition rides on the SIG booking.** Bots & Buyers EU runs **Oct 27-28** at MotorWorld Munich against **SIG Chicago, Oct 25-28**: **Petras→Chicago** with the SE, **Keil + Wagner→Munich**, **Heinzmann→the Munich demo stage**. The overlap is only Oct 27-28 of SIG's four days, so Keil can deliver the buy-side working session *and* reach Munich, but only if SIG schedules it on **Oct 25-26**. Make that a condition of the sponsorship, not something discovered in the agenda afterwards. If SIG will not commit, Petras delivers it and Keil does not travel. *(Petras, due Sep 15.)*

> **Correction (2026-08-31).** An earlier version of this sheet led with a Cologne collision. ProcureCon Europe against a Bots & Buyers Summit in Munich on Sep 23-24. Organizer verification showed that was wrong: the September own-event is **Bots & Buyers NYC on Sep 23** (free, application-only), which lands mid-gauntlet between Cologne and Boston and costs the **US pod**, not the DACH squad. The EU flagship moved to Munich on **Oct 27-28**. Cologne is unblocked; the clash moved to October.

## The month

| Date | Event | Note |
|---|---|---|
| Sep 1 | procure.ch Fachtagung, Langnau | Too late to sponsor |
| Sep 3 | KonnectHouse, London | |
| Sep 8-9 | Procurement & Supply Chain LIVE, London | London week, one trip |
| Sep 10 | CIPS Excellence Awards, London | |
| Sep 14-17 | SSON GBS cluster, San Diego | Early-bird lapsed; agentic bootcamp runs in-series |
| Sep 16-17 | Zip Forward, NYC | Intel only |
| Sep 17 | Argyle dinner, NYC | |
| Sep 21-23 | NASCES, Las Vegas | Bundle window |
| Sep 22-24 | **ProcureCon Europe, Cologne** | Unblocked, no DACH clash |
| Sep 23 | **Bots & Buyers NYC** | **Own franchise**, mid-gauntlet, US pod |
| Sep 28-30 | **ProcureCon East, Boston** | Lead sponsor, fee sunk |
| Sep 29-Oct 1 | BME Praxisforum, Frankfurt | Early-bird lapsed |
| Sep 30-Oct 1 | **DPW Amsterdam** | Award window lapsed |
| Sep 30 |, | **eLösungstage tier + BME-Symposium early-bird expire** |

## The 29 actions

### This week: by Wed Sep 2

| Due | Owner | Action |
|---|---|---|
| Sep 1 | DACH sales lead | **procure.ch** (event Sep 1), too late to sponsor; walk in or write off 2026 and target the Frühjahrstagung |
| Sep 1 | Nusmeier | **KonnectHouse London** (Sep 3), confirm attendance/speaking, then lock Frankfurt ~Jan 2027. A US West Coast 2026 edition is announced with dates unset, worth claiming early |
| Sep 2 | Petras | **SSON San Diego** (Sep 14-17, €38K), early-bird lapsed Jul 24; go/no-go at standard rate or roll into the SSON bundle. The "Agentic AI in Shared Services" bootcamp runs in-series, a zero-cost stage option, directly on-thesis |
| Sep 2 | Wagner | **P&SC LIVE London** (Sep 8-9), walk-in or skip; the London week is one trip |
| Sep 2 | Stäbe | **CIPS Awards** (Sep 10), confirm the ceremony admits non-shortlisted attendees; diarise the 2027 entry window |

### Next fortnight: by Thu Sep 10

| Due | Owner | Action |
|---|---|---|
| Sep 4 | Keil | **ProcureCon Europe Cologne** (Sep 22-24, €46K), book it; the WBR bundle with Boston closes with this event. No DACH clash after all |
| Sep 5 | Keil | **DPW Amsterdam** (Sep 30-Oct 1, €47K), establish whether the award application was filed; book sponsor tier + dinner regardless as defending champion |
| Sep 7 | Wagner | **ProcureCon East Boston** (Sep 28-30, €34K activation on €70K sunk), CPO dinner venue Sep 29, keynote slot, US customer announcement |
| Sep 7 | Schmid | **Argyle NYC dinner** (Sep 17), attend or skip |
| Sep 7 | Petras | **Bots & Buyers NYC** (Sep 23), get the US pod's applications in; free and application-only, and it sits between Cologne and Boston |
| Sep 8 | Stäbe | **BME Praxisforum** (Sep 29-Oct 1), early-bird lapsed Jul 31; standard rate or attend-only |
| Sep 8 | Petras | **NASCES** (Sep 21-23), the NAPES+NASCES bundle must be agreed before this event runs |
| Sep 10 | Petras | **NAPES 2027**, **verify the city first**: the Miami→Denver move is unconfirmed and the official site still shows 2026. No €35K commitment and no dinner venue until it is settled |
| Sep 10 | PMM | **Everest Engage Dallas** (Oct 5), book analyst 1:1s first, then decide |

### Mid-month: by Sun Sep 20

| Due | Owner | Action |
|---|---|---|
| Sep 14 | PMM | **Zip Forward NYC** (Sep 16-17), intel only, monitor remotely, never sponsor |
| Sep 15 | Keil | **SSOW DACH Berlin** (Nov 17-19), submit German keynote abstract, book sponsor tier |
| Sep 15 | Petras | **SIG Chicago** (Oct 25-28), check membership prerequisite, pitch a buy-side working session not a booth. Split settled (Petras→Chicago, Keil+Wagner→Munich, Heinzmann→demo stage); **make Oct 25-26 working-session scheduling a booking condition** so Keil can cover both rooms, draft ask in [18 §7](18-trigger-group-outreach-angles.md) |
| Sep 15 | Petras | **SSOW Autumn Amsterdam** (Oct 20-22), fold into the SSON bundle, then decide |
| Sep 15 | EU AE | **Deloitte SSC Brussels** (Oct 6-7), register 1-2 attendees, attend-only |
| Sep 15 | DACH sales lead | **BMÖ Vienna** (Oct 14-15), decide; pair with the SAP Connect Vienna partner angle if it lands |
| Sep 15 | Nusmeier | **SAP Connect Las Vegas** (Oct 5-7), partner angle only |
| Sep 20 | Wagner | **Maschinenbau-Gipfel** (Nov 10-11), sponsor or attend; separate Berlin week from Nov 17-19, needs its own dinner and budget line |
| Sep 20 | Schmid | **Reuters Sourcing USA** (Nov 3-4), inaugural and under-priced; sponsor or attend |
| Sep 20 | PMM | **Workday Rising / Oracle AI World** (Oct 12-15), skip unless named ABM targets attend |

### Month end: by Wed Sep 30

| Due | Owner | Action |
|---|---|---|
| Sep 30 | Stäbe | **BME-eLösungstage 2027**, sign the partner tier before the €300 discount lapses ← **hard deadline** |
| Sep 30 | Wagner | **61. BME-Symposium** (Nov 18-19), book the renewal before the **€100 early-bird lapses** ← **hard deadline**; reserve the Berlin dinner venue for Nov 18 |
| Sep 30 | DACH sales lead | **BME IT-Sourcing** (Oct 19-20), attend or speak; CIO/CISO gatekeeper audience |
| Sep 30 | DACH sales lead | **BVL Berlin** (Oct 21-22), decide; adjacent logistics-agent audience |
| Sep 30 | Stäbe | **GSA Festival**, 2026 edition missed; get on the 2027 list and diarise the early-bird |

**Closed on 2026-08-31, no longer an open action:** IZB Wolfsburg (Oct 27-29) is **dropped for 2026**. It collided with the Bots & Buyers Munich flagship, which commits the DACH squad; a biennial walk-the-floor with no booth, no dinner and no spend, so there was nothing to cancel. The auto-cluster rationale still stands, so reconsider at the 2028 edition.

## September load

Petras 6 · DACH sales lead 4 · Wagner 4 · Stäbe 4 · Keil 3 · PMM 3 · Schmid 2 · Nusmeier 2 · EU AE 1.

Petras gains the Bots & Buyers NYC application on top of the US pod's existing five, and owns both bundle negotiations. Keil carries only three items, but Cologne and Amsterdam are the two largest unbooked sums on the sheet.

## Caveats

Money figures are mid-range planning estimates pending organizer quotes, envelopes, not prices. Named owners on P1 events come from the "Team on site" column of the budget plan; role-based owners (DACH sales lead, PMM, EU AE) are proposed defaults derived from that plan's staffing rules, **not decisions anyone has signed up to**. Confirm them before holding anyone to a due date.

Dates here follow the organizer verification in [file 16](16-event-status-update.md) wherever the two sources disagreed. Where that file marks something unverified. NAPES' city, DPW New York's June dates, this sheet treats it as unverified too.
